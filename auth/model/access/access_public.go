...
func (s *service) CreateUserAccessToken(subject *Subject) (ret string, err error) {
	m, err := s.createTokenClaims(subject)
	if err != nil {
		return "", fmt.Errorf("[CreateUserAccessToken] 创建token Claim 失败，err: %s", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("[CreateUserAccessToken] 创建token失败，err: %s", err)
	}

	// 保存到redis
	err = s.saveTokenToCache(subject, ret)
	if err != nil {
		return "", fmt.Errorf("[CreateUserAccessToken] 保存token到缓存失败，err: %s", err)
	}
...
func (s *service) DeleteUserAccessToken(tk string) (err error) {
	// 解析token字符串
	claims, err := s.parseToken(tk)
	if err != nil {
		return fmt.Errorf("[DeleteUserAccessToken] 错误的token，err: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("[DeleteUserAccessToken] 清除用户token，err: %s", err)
	}
